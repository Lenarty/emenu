...
export default function Admin(props) {
    ...
    const handleChangeRestaurant = (event) => {
        ...
        props.onRestaurantIdChange(event.target.value)
    };
    ...
    console.log(props.items.restaurant_id)
    console.log(props.items.restaurant_name)
    console.log(props.items.restaurant_location)
    return (
                        ...
                        <List>
                            <ListItem>
                                <FormControl className={classes.formControl}>
                                    ...
                                    >
                                        ...
                                        {props.items.restaurant.map(item => (
                                            <MenuItem value={item.id}>{item.name}</MenuItem>
                                        ))}
                                    </Select>
                                    <FormHelperText>Please select the restaurant you want to update</FormHelperText>
                                </FormControl>
                                <Button onClick={() => props.onRestaurantUpdateButtonClick()} variant="contained" color="primary" className={classes.button}>
                                    Update restaurant
                                </Button>
                            </ListItem>
                            <ListItem>
                                {props.items.restaurant.map(item => (
                                    item.id === restaurant ? (
                                        <TextField
                                            onChange={(event) => props.onRestaurantNameChange(event.target.value)}
                                            id="standard-helperText"
                                            label={item.name}
                                            required
                                            helperText="Update restaurant name"
                                        />
                                    ):null))
                                }
                            </ListItem>
                            <ListItem>
                                {props.items.restaurant.map(item => (
                                    item.id === restaurant ? (
                                        <TextField
                                            onChange={(event) => props.onRestaurantLocationChange(event.target.value)}
                                            id="standard-helperText"
                                            label={item.location}
                                            required
                                            helperText="Update restaurant location"
                                        />
                                    ):null))
                                }
                            </ListItem>
